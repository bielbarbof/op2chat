...
function primePanelResources(){
  ...
  for(const resource of resources){
    void fetch(resource,{cache:'force-cache',credentials:'same-origin'}).catch(()=>{});
  }
}
...
async function hydrateInitialHistory(){
  let recent=[];
  try{
    const meta=await OBR.room.getMetadata();
    recent=Array.isArray(meta[RECENT_KEY]?.entries)?meta[RECENT_KEY].entries:[];
    cachedHistory=mergeEntries(cachedHistory,recent);
  }catch(error){console.error('OP2 Chat: falha ao carregar snapshot recente',error)}
  try{
    const scene=await loadSceneHistory();
    cachedHistory=mergeEntries(cachedHistory,scene);
    if(scene.length===0&&cachedHistory.length)await writeSceneHistory(cachedHistory);
  }catch(error){console.error('OP2 Chat: falha ao reconstruir histórico da cena',error)}
}
