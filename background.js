...
import {toggleChatPanel,warmChatPanelGeometry} from './panel.js';
...
async function hydrateInitialHistory(){const meta=await OBR.room.getMetadata();const recent=Array.isArray(meta[RECENT_KEY]?.entries)?meta[RECENT_KEY].entries:[];const scene=await loadSceneHistory();cachedHistory=mergeEntries(recent,scene);if(scene.length===0&&cachedHistory.length)await writeSceneHistory(cachedHistory)}
...
async function handle(event){
  ...
  if(d.type==='submit'&&d.entry){
    ...
    cachedHistory=mergeEntries(cachedHistory,[entry]);
    await persist(cachedHistory);
    await OBR.broadcast.sendMessage(CHAT_CHANNEL,{type:'persisted-entry',entry},{destination:'ALL'});
    return;
  }
  ...
  if(d.type==='delete'&&d.entryId){
    ...
    cachedHistory=cachedHistory.filter(e=>e.id!==d.entryId);
    await persist(cachedHistory);
    await OBR.broadcast.sendMessage(CHAT_CHANNEL,{type:'deleted',entryId:d.entryId},{destination:'ALL'});
  }
}
...
async function setup(){
  ...
  historyReady=identityReady.then(()=>role==='GM'?hydrateInitialHistory():undefined);
