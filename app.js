...
function setTheme(){const c=assignedId()?CHARACTERS[assignedId()]:null;document.documentElement.style.setProperty('--accent',c?.accent||(state.role==='GM'?'#c9c1b2':'#c6442b'));$('#identity').textContent=authorDisplay()}
...
function statusInfo(r){if(r.criticalSuccess)return['SUCESSO CRÍTICO','crit'];if(r.criticalFailure)return['FALHA CRÍTICA','crit'];if(r.success===true)return['SUCESSO','ok'];if(r.success===false)return['FALHA','fail'];return['ROLAGEM','']}

function renderFeed(){
  const feed=$('#feed');
  if(!state.entries.length){feed.innerHTML='<div style="padding:38px 12px;text-align:center;color:#716c64;font-family:Consolas,monospace;font-size:8px;letter-spacing:.12em">SEM REGISTROS // A INVESTIGAÇÃO AINDA NÃO COMEÇOU</div>';return}
  feed.innerHTML=state.entries.map(e=>{
    ...
    if(e.type==='message')return `<article class="message" style="--msg-accent:${e.accent||'#c6442b'}"><div class="message-head"><span class="author">${escapeHtml(e.authorName)}</span><span class="time">${relativeTime(e.createdAt)} ${del}</span></div><div class="message-text">${escapeHtml(e.text)}</div></article>`;
    if(e.type==='simple-roll'){
      const r=e.roll||{};return `<article class="message roll-card" style="--msg-accent:${e.accent||'#c6442b'}"><div class="message-head"><span class="author">${escapeHtml(e.authorName)}</span><span class="time">${relativeTime(e.createdAt)} ${del}</span></div><div class="roll-title">${escapeHtml(r.label||'Rolagem livre')}</div><div class="simple-values">${(r.values||[]).map(v=>`d${r.sides}: <b>${v}</b>`).join(' · ')}</div><div class="roll-summary"><div class="total"><span>RESULTADO</span><b>${r.total??0}</b></div><div></div><div></div></div></article>`;
    }
    const r=e.result||{};const [status,cls]=statusInfo(r);
    return `<article class="message roll-card" style="--msg-accent:${e.accent||'#c6442b'}"><div class="message-head"><span class="author">${escapeHtml(e.authorName)}</span><span class="time">${relativeTime(e.createdAt)} ${del}</span></div><div class="roll-title">${escapeHtml(r.label||'Teste')}</div><div class="dice-results">${(r.dice||[]).map(d=>`<div class="die-result"><span>${escapeHtml(d.source||'Dado')} // d${d.sides}</span><b>${d.value}</b></div>`).join('')}</div><div class="roll-summary"><div class="total"><span>RESULTADO</span><b>${r.total??0}</b></div><div class="metric"><span>RA</span><b>${r.ra??0}</b></div><div class="metric"><span>RB</span><b>${r.rb??0}</b></div></div><span class="roll-status ${cls}">${status}</span><div class="roll-meta">DT ${r.dt??'—'}${(r.dice||[]).length>3?' · SOMA DOS 3 MAIORES':''}</div></article>`;
  }).join('');
...
function getAccent(){const id=assignedId();return id?CHARACTERS[id].accent:(state.role==='GM'?'#c9c1b2':'#c6442b')}

function renderDiceTray(){
  const sides=[4,6,8,10,12,20];$('#diceButtons').innerHTML=sides.map(s=>`<button class="die-btn ${state.selectedSides===s?'active':''}" data-side="${s}">D${s}<small>${s===20?'RARO':'DADO'}</small></button>`).join('');
...
function setTestsOpen(open){state.testsOpen=open;$('#app').classList.toggle('tests-open',open);$('#testPanel').setAttribute('aria-hidden',String(!open));if(OBR.isAvailable)OBR.action.setWidth(open?950:430).catch(()=>{})}
function renderExtras(){
  ...
  $('#queuedExtras').innerHTML=state.manualExtras.length?state.manualExtras.map((s,i)=>`<span class="extra-chip">+d${s}<button data-remove-extra="${i}">×</button></span>`).join(''):'<span style="font-family:Consolas,monospace;font-size:7px;color:#706b63">NENHUM DADO EXTRA MANUAL</span>';
...
function renderTestPanel(){
  ...
  if(state.role==='GM'){
    $('#characterChooser').classList.remove('hidden');$('#characterChooser').innerHTML=`<select id="gmCharacter"><option value="">TESTE MANUAL</option>${Object.values(CHARACTERS).map(c=>`<option value="${c.id}">${c.name} // ${c.profile}</option>`).join('')}</select>`;$('#gmCharacter').value=state.testCharacterId||'';$('#gmCharacter').addEventListener('change',e=>{state.testCharacterId=e.target.value||null;renderTestPanel()});
  }else{$('#characterChooser').classList.add('hidden');state.testCharacterId=assigned}
  ...
  const attrs=Object.entries(c.attributes).map(([k,v])=>{const sides=stepDie(v,rt.stepMods?.[k]||0);return `<div class="attr-card"><span>${k==='fisico'?'Físico':k==='mente'?'Mente':'Emoção'}</span><b>d${sides}</b></div>`}).join('');
  const pending=(rt.pendingDice||[]).length?`<div class="pending-note">EFEITOS PREPARADOS // ${(rt.pendingDice||[]).map(d=>`+d${d.sides} ${escapeHtml(d.source)}`).join(' · ')}</div>`:'';
  const rows=SKILL_ORDER.map(key=>{const sk=c.skills[key];const attr=sk.attribute;const attrSides=stepDie(c.attributes[attr],rt.stepMods?.[attr]||0);return `<div class="skill-row"><span class="skill-name">${escapeHtml(labelSkill(c,key))}</span><span class="skill-die">d<b>${sk.die}</b></span><span class="plus">+</span><span class="attr-die">d<b>${attrSides}</b></span><button class="skill-roll" data-skill="${key}">ROLAR</button></div>`}).join('');
...
function renderManual(){
  const opts=[4,6,8,10,12,20].map(s=>`<option value="${s}">d${s}</option>`).join('');$('#manualTest').innerHTML=`<div class="manual-box"><h3>Teste com dois dados</h3><div class="manual-pair"><label>DADO A<select id="manualA">${opts}</select></label><label>DADO B<select id="manualB">${opts}</select></label></div><button id="manualRoll" class="primary">ROLAR TESTE</button></div>`;$('#manualA').value='6';$('#manualB').value='6';$('#manualRoll').addEventListener('click',rollManual);
}
...
async function rollSkill(c,key){
  ...
  const attrSides=stepDie(c.attributes[attr],(rt.stepMods?.[attr]||0)+state.attrStep);const skillSides=stepDie(sk.die,state.skillStep);
  const pending=eligiblePending(c,attr);const extras=[];const usedPending=[];
  ...
  const result=rollOp2Test({dice:[{sides:attrSides,source:attr==='fisico'?'Físico':attr==='mente'?'Mente':'Emoção',kind:'attribute'},{sides:skillSides,source:labelSkill(c,key),kind:'skill'},...extras],dt:Number($('#dtInput').value||7),label:`${labelSkill(c,key)} + ${attr==='fisico'?'Físico':attr==='mente'?'Mente':'Emoção'}`});
...
function rollManual(){
  const a=stepDie(Number($('#manualA').value),state.attrStep),b=stepDie(Number($('#manualB').value),state.skillStep);const extras=state.manualExtras.slice(0,2).map(s=>({sides:s,source:`Extra d${s}`,kind:'bonus'}));const result=rollOp2Test({dice:[{sides:a,source:'Dado A',kind:'attribute'},{sides:b,source:'Dado B',kind:'skill'},...extras],dt:Number($('#dtInput').value||7),label:'Teste manual'});sendEntry({id:makeId(),type:'test',createdAt:Date.now(),authorId:state.playerId,authorName:authorDisplay(),accent:getAccent(),result});state.manualExtras=[];state.attrStep=0;state.skillStep=0;renderExtras();updateStepLabels();
}
...
function onChatEvent(event){
  const d=event.data||{};
  if(d.type==='persisted-entry'&&d.entry){mergeAndRender([d.entry]);return}
  if(d.type==='deleted'&&d.entryId){state.entries=state.entries.filter(e=>e.id!==d.entryId);renderFeed();return}
  if(d.type==='history-part'&&d.targetConnectionId===state.connectionId){let bag=state.historyParts.get(d.requestId);if(!bag){bag={parts:[],total:d.total};state.historyParts.set(d.requestId,bag)}bag.total=d.total;bag.parts[d.index]=d.entries||[];if(bag.parts.filter(Boolean).length===bag.total){mergeAndRender(bag.parts.flat());state.historyParts.delete(d.requestId)}return}
}
...
async function setup(){
  ...
  setTheme();renderTestPanel();OBR.broadcast.onMessage(CHAT_CHANNEL,onChatEvent);OBR.room.onMetadataChange(meta=>{if(meta[ROOM_STATE_KEY]){state.roomState=normalizeRuntimeState(meta[ROOM_STATE_KEY]);setTheme();renderTestPanel()}if(Array.isArray(meta[RECENT_KEY]?.entries))mergeAndRender(meta[RECENT_KEY].entries)});await OBR.action.setWidth(430).catch(()=>{});await requestHistory();$('#loading').classList.add('hidden');
}
